...
bool JumpPointSearchMe::IsUnBlocked(int map[][COLUMNS], int row, int col)
{
	bool isCellBlocked = map[row][col] == 1;
	return isCellBlocked;
}

bool JumpPointSearchMe::InitChecks(int map[][COLUMNS], Pair src, Pair dest)
{
...
bool JumpPointSearchMe::IsDestination(int row, int col, Pair dest)
{
	bool isDestination = row == dest.first && col == dest.second;
	return isDestination;
}

double JumpPointSearchMe::CalculateHuristicValue(int row, int col, Pair dest)
{
	double huristic = ((double)sqrt((row - dest.first) * (row - dest.first) + (col - dest.second) * (col - dest.second)));
	return huristic;
}

void JumpPointSearchMe::TracePath(int map[][COLUMNS], Cell cellDetails[][COLUMNS], Pair dest)
{
	std::cout << "\nThe Path is ";
...
bool JumpPointSearchMe::Successor(int map[][COLUMNS], int row, int col, Pair dest)
{
	if (IsValid(curRow + row, curCol + col))
	{
		...
		while (!closedList[curRow + row][curCol + col] && IsUnBlocked(map, curRow + row, curCol + col))
		{
...
void JumpPointSearchMe::AStarSearch(int map[][COLUMNS], Pair src, Pair dest, bool diagonal)
{
	startingPos = src;
	endingPos = src;
